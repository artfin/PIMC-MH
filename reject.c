...
//                                   1.8912e-02, rel.diff: 1.04% (128bln)
// 300K: P = 4
//   SIGMA = 0.26
// acceptance rate: 1.03%
// UPDATED proposal, normal margin:  1.8990e-02, rel.diff: 1.46% (2000mln)
//                                   1.8970e-02, rel.diff: 1.35% (7040mln) 
//
// 300K: P = 8
//   SIGMA = 0.29 -- still fails, trying out 0.30
...
#define SIGMA 0.27 // the width of normally distributed for 'dx' around zeroth bead
...
int main(int argc, char *argv[])
{
    ...
    size_t iters = 1000;
    size_t packet_samples_count = 10000000; 
    ...
    for (size_t iter = 0; iter < iters; ++iter) {
        ...
        if (wrank == 0) {
            ...
            //total_var = ((total_samples_count-1)*total_var + (packet_samples_count-1)*packet_var) / 
            //             (total_samples_count + packet_samples_count - 1) + 
            //            total_samples_count*packet_samples_count*(total_mean - packet_mean)*(total_mean - packet_mean)/(total_samples_count + packet_samples_count) / 
            //            (total_samples_count + packet_samples_count- 1);
            ...
            for (int i = 1; i < wsize; ++i) {
                ...
                total_var = ((total_samples_count-1)*total_var + (packet_samples_count-1)*packet_var)/(total_samples_count-1 + packet_samples_count-1);
                // total_var = ((total_samples_count-1)*total_var + (packet_samples_count-1)*packet_var) / (total_samples_count + packet_samples_count - 1) + 
                //  total_samples_count*packet_samples_count*(total_mean - packet_mean)*(total_mean - packet_mean)/(total_samples_count + packet_samples_count)/(total_samples_count + packet_samples_count- 1);
                total_mean = (total_mean*total_samples_count + packet_samples_count*packet_mean) / (total_samples_count + packet_samples_count);
                total_samples_count += packet_samples_count;
            }
